...
nmea_options  =  [  '$GPGGA'
                    ...
                    '$GPRMC'
                    '$GPVTG'
                    %%'$GPZDA' Does not work with OATRC 2015 data
                    '$SDDBS'];
%
...
switch case_t
    ...
    case 6 %% GPVTG: Read course over ground and ground speed
        ...
        if(isempty(t_gspeed))
            data.groundspeedknot  =  NaN;
        else
            data.groundspeedknot  =  str2double(t_gspeed);
        end
        
        t_gspeed  =  fields(8,1:end);
        if(isempty(t_gspeed))
            data.groundspeedkph  =  NaN;
        else
            data.groundspeedkph  =  str2double(t_gspeed);
        end
